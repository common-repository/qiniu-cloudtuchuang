...
Description:  七牛云存储插件：在编辑器页面上传图片至七牛云对象存储
Author: 开心洋葱
...
class new_general_setting {
	...
	function register_fields() {
		register_setting( 'general', 'upload_url_path', 'esc_attr' );
		add_settings_field('fav_color', '<label for="upload_url_path">'.__('附件访问域名' ).'</label>' , array(&$this, 'fields_html') , 'general' );
	}

	function fields_html() {
		$value = get_option( 'upload_url_path', '' );
		echo '<input type="text" id="upload_url_path" name="upload_url_path" value="' . $value . '" />';
	}
...
function qiniu_cloudtuchuang_post_html(){
	$options = get_option('qiniu_options');
    $host = $options['host'];
	update_option( 'upload_url_path', $host );
    $bucket = $options['bucket'];
    $prefix = $options['prefix'];
    $accesskey = $options['accesskey'];
    $secretkey = $options['secretkey'];
